...
__int64  max_buf_size  = DEF_BUF_SIZE;
__int64  max_file_size = DEF_FILE_SIZE;
int  max_threads_num = DEF_NUM_THREADS;
__int64  auto_buf_size  = AUTO_BUF_SIZE;
...
unsigned char * fill_pattern = NULL;
char         * out_dir = NULL;
char         * LogFile = NULL;
int           delete_flag = 1;
int           verbose = 0;
int           quiet = 0;
int           check_work_dir_presence = 1;
...
int           fill_rand = 0;
int           stop_on_error = 0;
int           time_profiling = 0;
int           collect_summary_statistics = 1;
int           enable_random_sleep_in_thread = 0;
unsigned long int   g_passes_count = DEF_PASS_COUNT;
unsigned long int   us_wait_to_run_thread = 0;
int           emul_old_ftest = 0;
int           memory_test_only = 0;
int           retry_attempts = 1;
int           increment_filenames = 0;
int           auto_memory_test = 0;
int           use_unix_rng = 0;
int           use_mswin_rng = 0;


unsigned long int  g_total_exec_time = 0;

volatile struct stat_info  summary_stat;

const char prog_ver[] = "1.0.1";
const char rcs_id[] = "$Id: ftest3.c,v 1.8 2013-03-04 18:07:09+04 dstef Exp root $";
...
int unix_rand(void) // RAND_MAX assumed to be 32767
{
    next = next * 1103515245 + 12345;
    return (unsigned int)(next/65536) % 32768;
}

void unix_srand(unsigned int seed)
{
    next = seed;
}


void mswin_srand ( unsigned int seed )
{
...
void sleep(unsigned long int seconds)
{
   Sleep( seconds * 1000 );
}
#endif
 ...
 * Get next random number
 */
unsigned int next_rnd_num_word(void)
{
  unsigned int rn;
 ...
 */
void sync_file_changes(FILE * fp)
{
#if _WIN32
   _commit(fileno(fp));
#else
 ...
 */
void * time_exec_task_thread(void * arg)
{
   ...
   try_malloc_rw:    
     ...
     if (buffers[i].r_pbuf == NULL) {
       ...
       if (shadow_buffers_copy)
          shadow_buffers[my_thread_param.thread_id*max_elem_num+i].r_pbuf = 
	                                                    buffers[i].r_pbuf;
       if (verbose) printf( "Thread %d: Fill read_buffer %d\n",
                             my_thread_param.thread_id,i );     
       if (my_thread_param.fill_rand) {
         init_rnd_gen(i);
         len = buffers[i].msize;
...
void usage(void)
{
  ...
  fprintf( stderr, "Copyright (C) 2013 Dmitry Stefankov. All rights reserved.\n" );
 ...
 */
int  get_rlimit_value( int val, struct rlimit * p )
{
   int res;
 ...
 */
void  print_rlimit_value( const char * str, int value )
{
    struct rlimit   rlp;
    int             res;
    char            cur_buf[32];
    ...
    if (res == 0) {
        if (rlp.rlim_cur == RLIM_INFINITY) 
            strncpy(cur_buf, "INF", sizeof(cur_buf) );
        else
#if defined(__linux__) 
            snprintf( cur_buf, sizeof(cur_buf), "%lu", rlp.rlim_cur );
#else        
            snprintf( cur_buf, sizeof(cur_buf), "%llu", rlp.rlim_cur );
#endif            
        if (rlp.rlim_max == RLIM_INFINITY) 
            strncpy(max_buf, "INF", sizeof(max_buf) );
        else
#if defined(__linux__)
            snprintf( max_buf, sizeof(max_buf), "%lu", rlp.rlim_max );
#else
            snprintf( max_buf, sizeof(max_buf), "%llu", rlp.rlim_max );
#endif            
        snprintf( msg_buf, sizeof(msg_buf), "%s  cur=%s, max=%s\n", 
                  str, cur_buf, max_buf );
        printf( msg_buf );
    }
}
 ...
 */
void  print_rlimit_resources( void )
{
...
int  detect_x64_platform_bitness( void )
{
  int result = 0;
...
unsigned long int  detect_free_memory_in_MB( void )
{
  ...
  if (1) {
    ...
    if (verbose) printf("Maximum application address: 0x%lx\n", siSysInfo.lpMaximumApplicationAddress); 
...
/* Process command line  */  
  ...
  while( (op = getopt(argc,argv,"a:ABb:dDhEeFf:GgJ:j:L:lMmN:n:p:P:Oo:qrRs:ST:t:uvWw:yYzZ")) != -1)
   ...
   if (check_work_dir_presence) {
     ...
     s = getcwd( temp_dir_buf, sizeof(temp_dir_buf) );
     if (s == NULL) {
